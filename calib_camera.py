...
def main():
    try:
        ...
        if not os.path.exists(output_folder_merged):
            os.makedirs(output_folder_merged)
        if not os.path.exists(output_folder_combined_cam1):
            os.makedirs(output_folder_combined_cam1)
        if not os.path.exists(output_folder_combined_cam2):
            os.makedirs(output_folder_combined_cam2)
        ...
        cv2.namedWindow('Merged Cameras')
        cv2.namedWindow('Settings')

        cv2.createTrackbar('X Coordinate 1', 'Settings', 0, 1024, on_trackbar_x_change)
        cv2.createTrackbar('X Coordinate 2', 'Settings', 0, 1024, on_trackbar_x_change)
        ...
        while True:
            ...
            if grabResult1.GrabSucceeded() and grabResult2.GrabSucceeded():
                ...
                x1 = cv2.getTrackbarPos('X Coordinate 1', 'Settings')
                if combined_image_cam1.shape[1] > 1024:  
                    combined_image_cam1[:, 1024] = 255  

                if combined_image_cam1.shape[1] > x1:  
                    combined_image_cam1[:, x1] = 255 
                    combined_image_cam1[:, -x1-1]= 255
                cv2.imshow('Combined Horizontal 1', combined_image_cam1)  

                combined_image_cam2 = cv2.vconcat(frames_cam2)
                x2 = cv2.getTrackbarPos('X Coordinate 2', 'Settings')
                if combined_image_cam2.shape[1] > 1024:  
                    combined_image_cam2[:, 1024] = 255  

                if combined_image_cam2.shape[1] > x2:  
                    combined_image_cam2[:, x2] = 255
                    combined_image_cam2[:, -x2-1]= 255
                cv2.imshow('Combined Horizontal 2', combined_image_cam2) 
                ...
                gap = np.zeros((max_height, 0), dtype=np.uint8)
